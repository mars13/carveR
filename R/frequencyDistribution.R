  ...
  tagList(sidebarLayout(
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        box(
          ...
          p(
            "The dashboard will only become active once at least one file has been uploaded."
          )
        )
      ),
      width = 3
    ...
    mainPanel(
      ...
      box(
        ...
        status = "primary",
        width = 12,
        shinycssloaders::withSpinner(plotlyOutput(ns("distPlot")),
                                     type = 8)
      ),
      br(),
      box(
...
frequencyDistribution_server = function(id,
  ...
  moduleServer(id,
               function(input, output, session) {
                 ...
                 output$sidebar_status = renderText(if (as.numeric(datasets$datarows) != 0) {
                   "OK"
                 } else {
                   "WARNING"
                 })
                 ...
                 output$extraCheckbox = renderUI({
                   if (as.numeric(datasets$datarows) != 0) {
                     ...
                     bs4Callout(title = "Dashboard will display after uploading the first dataset.",
                                status = "danger",
                                elevation = 1)
                   }
                 })
                 ...
                 observe({
                   if (!is.null(columns$col_names)) {
                     columns$order = unlist(lapply(columns$col_names, function(x)
                       input[[paste0(x, "_radio")]]))
                   }
                 })

                 output$sortColumns = renderText(if (!is.null(columns$col_names)) {
                 ...
                 output$distPlot <- renderPlotly({
                   ...
                   theme_colors = c("#F6CA28", "#34AF88", "#E21A1A", "#5BC8C8")
                   plot_data = modify_plot_data(
                     data_filt(),
                   ...
                   plotly::plot_ly(
                     ...
                     plotly::layout(
                       xaxis = list(rangeslider = list(), title = ''),
                 ...
                 output$download_tsv = downloadHandler(
                   filename = function() {
                     paste("carver_rank-", Sys.Date(), ".tsv", sep = "")
                   },
                   content = function(file) {
                     outrank = rank()
                     outrank$rank = rownames(rank())
                     write.table(
                       outrank,
                       file,
                       sep = "\t",
