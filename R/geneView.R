geneView_ui <- function(id) {
  ...
  tagList(
    fluidRow(
      column(2, selectInput(ns("select_gene"), "Select gene", choices = "")),
    ),
    fluidRow(
      box(
        title = "Gene stats",
        ...
        shinycssloaders::withSpinner(
          ...
          column(width = 6, style = "min-width: 250px;",
                 plotlyOutput(ns("consequencePlot")))
          ),
          type = 8)
      ),
      box(
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$variantPlot = renderPlotly({
        plotly_piechart(gene_data(), "variant_type", theme_colors, "Variant type")
      })

      output$consequencePlot = renderPlotly({
        plotly_barplot(gene_data(), "consequence_summary", theme_colors[1], "Variant consequence")
      })
      ...
      output$varTable = DT::renderDT(server = T, {
        cols = c("dataset",
                 "variant_type",
                 "consequence_summary",
                 colnames(gene_data())[!colnames(gene_data()) %in% c("variant_id", "sample_id", "gene", "variant_type", "consequence_summary")])
        datatable(
          gene_data()[,cols],
          filter = "top",
          rownames = F,
        ...
        ) %>% formatStyle(columns = c(1:3), target='cell', backgroundColor =  colorspace::lighten(theme_colors[1], 0.9),)
      })
      ...
      output$download_tsv = downloadHandler(
        filename = function() {
          paste("carver_", unique(gene_data()$gene),"_variants-", Sys.Date(), ".tsv", sep="")
        },
        content = function(file) {
          write.table(gene_data(), file, sep = "\t", quote = F, col.names = T, row.names = F)
        }
      )
